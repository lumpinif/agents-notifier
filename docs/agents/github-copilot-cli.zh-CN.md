...
GitHub Copilot CLI 官方支持从当前工作目录的 `.github/hooks/*.json` 加载 hooks。Agents Notifier 走的是官方 `notification` hook 路径，并且只接收你在命令里明确传入的标题和正文。
...
Agents Notifier 只需要 Copilot CLI hook 运行这一条命令：
