...
Aider 官方支持 notifications，也支持自定义 `notifications_command`。Agents Router 走这条官方 command 路径，并且只接收你在命令里明确传入的标题和正文。
...
Agents Router 只需要 Aider 运行这一条命令：
...
- Aider notification command 的 shell 环境里是否能找到 `agents-router`。
