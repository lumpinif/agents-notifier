...
当你想通过自己的 SMTP server 或邮件服务商，把 Agents Router 通知发送成纯文本邮件时，就用 Email SMTP。

Agents Router 做的是 SMTP 发信。它不收邮件、不读 inbox、不管理 OAuth，也不发送附件。
...
Agents Router 会保存 provider、启动本地 service，并通过真实 agent 事件使用的同一条 service route 发送一封测试邮件。

## Answer Detail

Email SMTP 不像手机或聊天 provider 那样有很小的消息长度限制。

Agents Router 对 Email SMTP 允许 `Preview` 或 `Full Answer`。

## Prompt Detail

Agents Router 对 Email SMTP 允许 prompt detail，但要注意隐私。邮件可能经过你的邮件服务商服务器，并保存在收件人 inbox 里。
