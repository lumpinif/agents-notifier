...
当你想把 Agents Router 通知发到一个 Discord channel 时，就用 Discord。

Agents Router 使用 Discord Incoming Webhooks。一个 webhook 会发到一个 channel。
...
Agents Router 会对 Discord 固定使用 `Preview` answer detail。

Discord webhook `content` 最多 2000 个字符。完整回答可能很长，所以 Agents Router 会让 Discord 通知保持短小，保证投递更可靠。

## Prompt Detail

Agents Router 会对 Discord 禁用 prompt detail。

Discord webhook `content` 最多 2000 个字符。Prompt 可能很长，也可能包含私人信息，所以 Agents Router 不会把 prompt 放进 Discord 通知里。
...
Agents Router 会用 `wait=true` 发送，让 Discord 确认已创建消息。它也会禁用自动 mention，所以通知正文里即使包含 `@everyone`，也不会 ping 整个 server。

如果某条格式化后的 Discord 通知太长，Agents Router 会在发送前让这次 Discord 投递失败。它不会偷偷截断你的消息。

Agents Router 会对 Discord 始终使用 `Preview` answer detail。
