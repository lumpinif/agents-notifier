...
当你想把 Agents Router 通知发到一个 Telegram 私聊、群组或频道时，就用 Telegram。

Agents Router 使用 Telegram Bot API 的 `sendMessage` 方法，只发送纯文本。
...
Agents Router 会对 Telegram 固定使用 `Preview` answer detail。

Telegram Bot API text message 最多 4096 个字符。完整回答可能很长，所以 Agents Router 会让 Telegram 通知保持短小，保证投递更可靠。

## Prompt Detail

Agents Router 会对 Telegram 禁用 prompt detail。

Prompt 可能很长，也可能包含私人信息。Telegram Bot API text message 最多 4096 个字符，所以 Agents Router 不会把 prompt 放进 Telegram 通知里。
...
Agents Router 通过 `sendMessage` 发送 Telegram 消息，字段是 `chat_id` 和 `text`。

如果某条格式化后的 Telegram 通知太长，Agents Router 会在发送前让这次 Telegram 投递失败。它不会偷偷截断你的消息。

Agents Router 会对 Telegram 始终使用 `Preview` answer detail。
